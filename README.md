...
Debug HUD 是一个 Next.js 应用。为了让用户本地零配置启动 HUD，我们会在发布包中一并包含 HUD 的生产构建产物：
- 包内包含 `src/hud/server.js`（HUD 服务器入口）
- 包内包含 `src/hud/.next/`（Next.js 构建输出）

因此，用户安装 `vdt-mcp` 后无需再在本地构建 HUD；CLI 使用 `--hud` 时会直接启动内置的 HUD。你不需要让用户下载源码或执行 `next build`。

### 发布包包含内容

发布到 npm 的包包含以下关键目录：
- `dist/**`：编译后的 MCP 服务器与 CLI
- `src/hud/server.js`：HUD 服务器
- `src/hud/.next/**`：HUD 的 Next.js 构建产物
- `src/hud/public/**`：HUD 的静态资源（如有）

这些已在 `package.json#files` 中显式列出，避免额外文件被忽略。
...
2. 本地验证 HUD 构建：
```bash
pnpm run hud:install
pnpm run hud:build
```

> 注意：发布时会自动执行 `prepack`，它会顺序运行：`build` → `hud:install` → `hud:build`，以生成并打包 `src/hud/.next/`。
...
用户只需安装并带上 `--hud` 参数即可启动内置 HUD：
```bash
npm install -g vdt-mcp
vdt-mcp --hud
```

或在项目内：
```bash
npm install vdt-mcp --save-dev
npx vdt-mcp --hud
```

可选参数：
- `--hud-port=4000` 指定 HUD 端口
- 直接运行 MCP 而不启用 HUD：`npx vdt-mcp`

### 常见问题

- 如果终端面板报错多发生在 `node-pty` 编译阶段。我们在 `postinstall` 中已尝试自动修复；若仍失败，参考日志提示运行手动重建命令，或在不需要终端面板时忽略。
- 若用户机器未安装 `pnpm`，发布脚本会回退到 `npm`。包内已包含 HUD 构建，用户侧无需 `pnpm`。
